...
description: >
  Requirements grilling session. Interrogates the developer about a plan before
  any spec or code is written. Explores the codebase first, asks one question at
  a time with a recommended answer, updates CONTEXT.md inline as domain terms
  crystallise, and proposes ADRs only for hard-to-reverse decisions.
