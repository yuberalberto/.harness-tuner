...
#### If an Issue File Exists

When an issue ID is provided as argument (e.g. `/tdd-cycle 003-02`) or is
referenced in conversation context, read the issue file at
`.specs/<NNN>-<feature>/issues/<NNN>-<NN>-<slug>.md`.

Extract the **Acceptance Criteria** as the planned behaviors to test. Present
them to the user and ask: "These are the behaviors I'll test — remove any you
want to skip, or add others." Do not re-derive from scratch.

Also read the `What to Build` section to understand the interface implied by the
slice, and use it to answer the interface design questions below.

#### If No Issue File Exists
