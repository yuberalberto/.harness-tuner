---
name: audit
description: Run a comprehensive pre-push audit covering dependency vulnerabilities, linters, secrets, dangerous patterns, and tests. Collects everything and presents a consolidated report.
---

# Audit

Run a comprehensive pre-push audit covering dependency vulnerabilities, linters,
secrets, dangerous patterns, and tests.

**Critical:** Do not stop on intermediate findings. Collect everything and
...
If no manifest is found, skip dependency and test steps and note it in the
report.
...
- Common credential patterns: `AKIA[0-9A-Z]{16}`, `ghp_[A-Za-z0-9]{36}`,
  `sk-[A-Za-z0-9]{20,}`, private key headers (`-----BEGIN`)
- Sensitive files staged for commit: `.env*`, `*.pem`, `id_rsa*`, `*.pfx`,
  `credentials.json`
...
- **JS/TS**: `eval(`, `new Function(`, `innerHTML =`, `dangerouslySetInnerHTML`,
  `document.write`
- **Python**: `eval(`, `exec(`, `pickle.loads`, `subprocess.*shell=True`,
  `yaml.load` without SafeLoader
- **Shell/any**: hardcoded IPs, plaintext `http://` endpoints, SQL string
  concatenation, OS command injection via unsanitized input
...
- Flag SOLID violations, new tech debt, missing error handling at boundaries,
  broken invariants.
...
- **Critical** (secrets, failed tests, high-severity vulns): recommend addressing
  before push.
