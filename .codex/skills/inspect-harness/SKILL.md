...
description: >
  Report what is currently loaded in the project's Codex harness across
...
Report what is currently configured in your project's Codex harness across all 5 layers.
...
Read `.codex/rules/*.md` files and report:
- File path
...
Glob `.codex/skills/*/SKILL.md` (or `.codex/skills/*/` directories). For each:
...
Read `.codex/settings.json` and extract:

**Hooks section** (if present):
- List event → script path mappings
- Example: `"SessionStart" → "./.codex/hooks/engram-session-start.ps1"`
...
Report the native Codex subagents available for this harness:
...
## Rules Layer
- `.codex/rules/identity.md` (~2 KB)
- `.codex/rules/engram.md` (~3 KB)
- `.codex/rules/sdd-process.md` (~2 KB)
...
## Hooks
- SessionStart → ./.codex/hooks/engram-session-start.ps1
- SessionEnd → ./.codex/hooks/engram-session-end.ps1
...
...
**Navigation**: Include full `.codex/` paths so the user can click/navigate to any file from the report.
